...
export default function HazardReporting() {
  ...
  const loadRecentHazards = async () => {
    try {
      ...
      if (response.success) {
        ...
        // Hazards API not available - use empty list
        setHazards([])
      }
    } catch (error) {
      // Silently handle error - hazards feature is optional
      console.log('Hazards API not available')
      setHazards([])
    } finally {
      setLoading(false)
    }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      console.log('Hazards API not available:', error)
      setToast({
        message: 'Hazard reporting is currently unavailable. Please try again later.',
        type: 'error'
      })
    }
  }
                    ...
                    <div>
                      ...
                      <div className="text-xs text-gray-500 mt-1">
                        ...
                        {formData.description.length > 0 && formData.description.length < 10 && (
                          <span className="text-red-500 ml-2">⚠️ Need {10 - formData.description.length} more</span>
                        )}
                      </div>
                    </div>
